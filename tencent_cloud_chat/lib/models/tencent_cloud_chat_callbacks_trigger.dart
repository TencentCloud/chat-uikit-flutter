...
class TencentCloudChatCallbacksTrigger {
  ...
  void _initDefaultV2TimSDKListener (){
    defaultV2TimSDKListener ??= V2TimSDKListener(
      ...
      onAllReceiveMessageOptChanged: (V2TimReceiveMessageOptInfo receiveMessageOptInfo){
        for (final callbacks in callbacksList) {
          callbacks.onSDKEvent?.onAllReceiveMessageOptChanged(receiveMessageOptInfo);
        }
      },
  ...
  void onUserNotificationEvent(TencentCloudChatComponentsEnum component, TencentCloudChatUserNotificationEvent event) {
    TencentCloudChat.instance.logInstance.console(
      componentName: componentName,
      logs: json.encode({
